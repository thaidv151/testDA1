...
class HomeController
{
    ...
    public function postDangKy()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            $listUsers = $this->model->getAllUser();
            $error = [];
            if(strlen($ten_dang_nhap) <= 5){
                $error['ten_dang_nhap'] = "Tài khoản phải dài hơn 5 ký tự";
            }
            if (empty($ten_dang_nhap)) {
                $error['ten_dang_nhap'] = "Không để trống tên đăng nhập";
            }
            if(strlen($mat_khau) <= 7){
                $error['mat_khau'] = "Mật khẩu phải lớn hơn 7 ký tự";
            }
            if (empty($mat_khau)) {
            ...
            if(strlen($ten_dang_nhap) > 30){
                $error['ten_dang_nhap'] = "Tài khoản phải ngắn hơn 30 ký tự";
            }
           
           
            
            if(strlen($mat_khau) > 30){
                $error['mat_khau'] = "Mật khẩu không lớn hơn 30 ký tự";
            }
            
           
            $regex_phone = "^\\+?[1-9][0-9]{7,14}$";

            if (!preg_match($so_dien_thoai, $regex_phone)) {
